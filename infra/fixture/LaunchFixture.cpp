...
infra::domain::SatResult exit_code_to_sat_result(int exit_code) {
  switch (exit_code) {
    case 0: return infra::domain::UNSAT;
    case 1: return infra::domain::SAT;
    case 2: return infra::domain::UNKNOWN;
    default: return infra::domain::SAT_ERROR;
  }
}
...
void LaunchFixture::_prepare_resources() {
  ...
  for (std::string const& test_group : config.test_groups) {
    for (auto const& entry : std::filesystem::recursive_directory_iterator(
             cnf_path / test_group, std::filesystem::directory_options::follow_directory_symlink)) {
