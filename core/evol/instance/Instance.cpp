...
uint32_t Instance::num_vars() const noexcept { return _preprocess->var_view().size(); }
...
core::domain::Vars const& Instance::get_vars() const noexcept { return _vars; }

Instance* Instance::clone() { return new Instance(*this); }
...
void Instance::_calc_fitness() { _calc_fitness(_sampling_config().base_samples, _sampling_config().max_scale_steps); }
...
bool Instance::is_cached() const noexcept { return _cached || _shared->cache.get(_vars.get_mask()).has_value(); }
...
uint32_t Instance::size() const noexcept { return _vars.size(); }

instance::SharedData::SamplingConfig& Instance::_sampling_config() noexcept { return _shared->sampling_config; }

util::LRUCache<std::vector<bool>, Fitness>& Instance::_cache() noexcept { return _shared->cache; }

uint64_t& Instance::_inaccurate_points() noexcept { return _shared->search_space.inaccurate_visited_points; }

core::domain::VarView const& Instance::_var_view() const noexcept { return _preprocess->var_view(); }

bool operator<(Instance& a, Instance& b) { return a.fitness() < b.fitness(); }
