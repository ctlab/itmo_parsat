...
bool operator<(Fitness const& a, Fitness const& b) {
  ...
  if (a.can_calc() && b.can_calc()) { return (double) a < (double) b; }

  if (a.size == 0 || a.rho < EPS || a.size > b.size + CRIT_SZ_DIFF) { return false; }
  if (b.size == 0 || b.rho < EPS || b.size > a.size + CRIT_SZ_DIFF) { return true; }
