...
static void run_propagate(
    core::sat::prop::Prop& prop, core::domain::USearch p_search, prop_assignments_t) {
  prop.prop_assignments(std::move(p_search), [](auto, auto) { return true; });
}

static void run_propagate(
    core::sat::prop::Prop& prop, core::domain::USearch p_search, prop_with_dummy_t) {
...
static void run_propagate(
    core::sat::prop::Prop& prop, Minisat::vec<Minisat::Lit> const& vars, prop_tree_t) {
  prop.prop_tree(vars, 0);
}

template <typename T, typename TestType, typename VarsF, typename... Args>
void test_search(benchmark::State& state, TestType type, VarsF const& vars_f, Args&&... args) {
...
void tree_search(benchmark::State& state, Args&&... args) {
  test_search<T>(
      state, prop_tree,
      [](int size, int nvars) { return common::to_mini(common::gen_vars(size, nvars)); },
      std::forward<Args>(args)...);
}
...
BENCHMARK(BM_full_minisat)
    ->ArgsProduct(
        {benchmark::CreateDenseRange(0, 20, 4), benchmark::CreateDenseRange(0, N_INPUTS - 1, 1)});
