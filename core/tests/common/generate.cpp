...
core::lit_vec_t gen_assumption(core::domain::VarView const& var_view, int min_size, int max_size) {
  int max_var = (int) var_view.size() - 1;
  auto assumption = gen_assumption(min_size, max_size, max_var);
  for (int i = 0; i < assumption.size(); ++i) {
    assumption[i] = Mini::mkLit(var_view[Mini::var(assumption[i])], Mini::sign(assumption[i]));
  }
  return assumption;
}
