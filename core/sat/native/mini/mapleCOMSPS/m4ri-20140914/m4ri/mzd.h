...
typedef struct mzd_t {
  ...
  mzd_block_t* blocks; /*!< Pointers to the actual blocks of memory containing the values packed
                          into words. */
  word**
      rows; /*!< Address of first word in each row, so the first word of row i is is m->rows[i] */
...
mzd_t* mzd_init_window(
    mzd_t* M, rci_t const lowr, rci_t const lowc, rci_t const highr, rci_t const highc);
...
static inline mzd_t const* mzd_init_window_const(
    mzd_t const* M, rci_t const lowr, rci_t const lowc, rci_t const highr, rci_t const highc) {
...
static inline void _mzd_row_swap(
    mzd_t* M, rci_t const rowa, rci_t const rowb, wi_t const startblock) {
...
static inline void mzd_col_swap_in_rows(
    mzd_t* M, rci_t const cola, rci_t const colb, rci_t const start_row, rci_t const stop_row) {
  if (cola == colb)
    return;
...
static inline void mzd_xor_bits(
    mzd_t const* M, rci_t const x, rci_t const y, int const n, word values) {
...
static inline void mzd_and_bits(
    mzd_t const* M, rci_t const x, rci_t const y, int const n, word values) {
 ...
 */
mzd_t* mzd_submatrix(
    mzd_t* S, mzd_t const* M, rci_t const lowr, rci_t const lowc, rci_t const highr,
    rci_t const highc);
...
static inline void mzd_combine_even_in_place(
    mzd_t* A, rci_t const a_row, wi_t const a_startblock, mzd_t const* B, rci_t const b_row,
...
static inline void mzd_combine_even(
    mzd_t* C, rci_t const c_row, wi_t const c_startblock, mzd_t const* A, rci_t const a_row,
    wi_t const a_startblock, mzd_t const* B, rci_t const b_row, wi_t const b_startblock) {
  wi_t wide = A->width - a_startblock - 1;
  word* a = A->rows[a_row] + a_startblock;
  word* b = B->rows[b_row] + b_startblock;
 ...
 *
 */
static inline void mzd_combine(
    mzd_t* C, rci_t const c_row, wi_t const c_startblock, mzd_t const* A, rci_t const a_row,
    wi_t const a_startblock, mzd_t const* B, rci_t const b_row, wi_t const b_startblock) {
