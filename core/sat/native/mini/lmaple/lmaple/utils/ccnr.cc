    ...
    for (i = 1; i < lssolver->goodvar_stack_fill_pointer; ++i) {
      ...
      if (lssolver->score[v] > lssolver->score[best_var])
        best_var = v;
          ...
          lssolver->score[v] == lssolver->score[best_var] && lssolver->time_stamp[v] < lssolver->time_stamp[best_var])
        best_var = v;
    }
...
bool local_search(CCAnr* lssolver) {
  ...
  for (lssolver->step = 0; lssolver->step < lssolver->max_flips; lssolver->step++) {
    ...
    if (lssolver->unsat_stack_fill_pointer < lssolver->best_cost) {
      ...
      for (int i = 1; i <= lssolver->num_vars; ++i)
        lssolver->best_soln[i] = lssolver->cur_soln[i];
    }
    if (lssolver->unsat_stack_fill_pointer == 0)
      return true;
    if (lssolver->mems_left < 0 && lssolver->step > 1000)
      return false;
...
void init(CCAnr* lssolver, char* soln) {
  ...
  for (v = 1; v <= lssolver->num_vars; v++) {
    if (lssolver->fix[v] == 0) {
      if (soln == NULL) {
        if (ccanr_rand(lssolver, 2) == 1)
          lssolver->cur_soln[v] = 1;
        else
          lssolver->cur_soln[v] = 0;
      } else {
  ...
  for (v = 1; v <= lssolver->num_vars; v++) {
    ...
    for (i = 0; i < lit_count; ++i) {
      ...
      if (lssolver->sat_count[c] == 0)
        lssolver->score[v]++;
      else if (lssolver->sat_count[c] == 1 && lssolver->var_lit[v][i].sense == lssolver->cur_soln[v])
        lssolver->score[v]--;
    }
  }
...
void update_after_build(CCAnr* lssolver) {
  ...
  if (lssolver->unitclause_queue_end_pointer > 0)
    preprocess(lssolver);
}
...
void unit_propagation(CCAnr* lssolver) {
  ...
  for (lssolver->unitclause_queue_beg_pointer = 0;
       lssolver->unitclause_queue_beg_pointer < lssolver->unitclause_queue_end_pointer;
    ...
    if (lssolver->fix[uc_var] == 1) {
      if (uc_sense != lssolver->cur_soln[uc_var])
...
void ccanr_merseene_init(CCAnr* lssolver, int seed) { ccanr_Mersenne_init_with_seed(&(lssolver->randgen), seed); }

int ccanr_rand(CCAnr* lssolver, int bound) { return ccanr_Mersenne_next(&(lssolver->randgen), bound); }
...
void ccanr_Mersenne_init_with_seed(ccanr_randgen* randgen, int seed) {
  ...
  for (randgen->mti = 1; randgen->mti < ccanr_Mersenne_N; randgen->mti++) {
    randgen->mt[randgen->mti] =
        (1812433253UL * (randgen->mt[randgen->mti - 1] ^ (randgen->mt[randgen->mti - 1] >> 30)) + randgen->mti);
