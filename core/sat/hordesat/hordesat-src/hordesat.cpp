...
int main(int argc, char** argv) {
  ...
  if (params.getFilename() == NULL || params.isSet("h")) {
    ...
    puts("Parameters:");
    puts(
        "        -d=0...7\t diversification 0=none, 1=sparse, 2=dense, 3=random, "
        "4=native(plingeling), 5=1&4, 6=sparse-random, 7=6&4, default is 1.");
    puts(
        "        -e=0,1,2\t clause exchange mode 0=none, 1=all-to-all, 2=log-partners, default is "
        "1.");
  ...
  log(0, "Running HordeSat ($Revision: 46 $) on %s rank %d/%d input %s with parameters: ", hostname,
      mpi_rank, mpi_size, params.getFilename());
  params.printParams();
  ...
  if (exchangeMode == 0) {
    ...
    switch (exchangeMode) {
      case 1:
        sharingManager =
            new AllToAllSharingManager(mpi_size, mpi_rank, solvers, params.isSet("fd"));
        log(1, "Initialized all-to-all clause sharing.\n");
        break;
      case 2:
        sharingManager = new LogSharingManager(mpi_size, mpi_rank, solvers, params.isSet("fd"));
  ...
  for (int i = 0; i < solversCount; i++) {
    ...
    log(1, "thread-stats node:%d/%d thread:%d/%d props:%lu decs:%lu confs:%lu mem:%0.2f\n",
        mpi_rank, mpi_size, i, solversCount, st.propagations, st.decisions, st.conflicts,
        st.memPeak);
  ...
  log(1,
      "node-stats node:%d/%d solved:%d res:%d props:%lu decs:%lu confs:%lu mem:%0.2f shared:%lu "
      "filtered:%lu\n",
      mpi_rank, mpi_size, finalResult != 0, finalResult, locSolveStats.propagations,
      locSolveStats.decisions, locSolveStats.conflicts, locSolveStats.memPeak,
      locShareStats.sharedClauses, locShareStats.filteredClauses);
  // Global statistics
  ...
  MPI_Reduce(
      &locSolveStats.propagations, &globSolveStats.propagations, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
      MPI_COMM_WORLD);
  MPI_Reduce(
      &locSolveStats.decisions, &globSolveStats.decisions, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
      MPI_COMM_WORLD);
  MPI_Reduce(
      &locSolveStats.conflicts, &globSolveStats.conflicts, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
      MPI_COMM_WORLD);
  MPI_Reduce(
      &locSolveStats.memPeak, &globSolveStats.memPeak, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  SharingStatistics globShareStats;
  MPI_Reduce(
      &locShareStats.sharedClauses, &globShareStats.sharedClauses, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
      MPI_COMM_WORLD);
  MPI_Reduce(
      &locShareStats.importedClauses, &globShareStats.importedClauses, 1, MPI_UNSIGNED_LONG,
      MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(
      &locShareStats.filteredClauses, &globShareStats.filteredClauses, 1, MPI_UNSIGNED_LONG,
      MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(
      &locShareStats.dropped, &globShareStats.dropped, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
      MPI_COMM_WORLD);

  if (mpi_rank == 0) {
    log(0,
        "glob-stats nodes:%d threads:%d solved:%d res:%d rounds:%d time:%.2f mem:%0.2f MB "
        "props:%.2f decs:%.2f confs:%.2f "
        "shared:%.2f imported:%.2f filtered:%.2f dropped:%.2f\n",
        mpi_size, solversCount, globalResult != 0, globalResult, round, searchTime,
        globSolveStats.memPeak, globSolveStats.propagations / searchTime,
        globSolveStats.decisions / searchTime, globSolveStats.conflicts / searchTime,
        globShareStats.sharedClauses / searchTime, globShareStats.importedClauses / searchTime,
        globShareStats.filteredClauses / searchTime, globShareStats.dropped / searchTime);
  }
