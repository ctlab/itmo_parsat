...
void test_propagation(
    ...
    std::function<
        void(Minisat::SimpSolver&, core::sat::prop::Prop& prop)> const& f) {
...
void test_propagation_bool(
    ...
    std::function<
        ...
        prop) {
  test_propagation(
      problem, prop_config, [&](auto& base_simp_solver, auto& prop_engine) {
        common::iter_assumptions(
            [&](auto const& assumption) {
              Mini::vec<Mini::Lit> dummy;
              bool expected =
                  !base_simp_solver.prop_check(assumption, dummy, 0);
      ...
      problem, prop_config, [&](auto& base_simp_solver, auto& prop_engine) {
        common::iter_assumptions(
            [&](auto& assumption) {
              ...
              uint64_t expected = num_conflicts_in_subtree(
                  base_simp_solver, assumption, head_size);
